    ...
    for (const row of upsertData) {
      try {
        ...
        if (existing) {
          ...
          logger.info(`🔄 Updated device=${row.device_id} | ${existing.timestamp} → ${row.timestamp}`);
        ...
        failCount++;
        logger.error(`❌ Upsert failed | device=${row.device_id} | reason: ${extractErrorMessage(err)}`);
